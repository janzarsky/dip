...
The problems described above mean that the analyzer will never discard already
...
\todo{For dynamic analysis of contracts for concurrency, only a subset of all
events is required. The RoadRunner framework should be configured or modified to
...
The main class, \texttt{ContractAnalyzer}

\subsection{Tracking of Target and Spoiler Instances}

\todo{FiniteAutomaton, Instance, InstanceCollection, Window}

\todo{For each thread, a \texttt{Window} object is kept during the analysis. It
...
\todo{When a window is created, for each target and spoiler, an empty
\emph{template} instance is added that will be used for detecting new instances
in the trace window. These instances will remain unchanged during the lifetime
of the window.}

\todo{On method entry, the analyzer will try to advance all instances in a
window.  When a template instance can be advanced, it means that a new instance
is detected. The template instance is copied, the value of parameters is stored
in the instance, and the instance is advanced to the next state. When an already
running instance can be advanced, the analyzer must check if the method
arguments match those stored in the instance.}
{\color{blue}\lipsum[2]}

\subsection{Detection of Contract Violation}

\todo{On method exit, for all target instances that were just fully accepted, it
is checked if there is a spoiler instance in other windows that can violate this
...
The contract analyzer is implemented as a subclass of the \texttt{Tool} class.
The following methods are overridden:
\begin{itemize}
    \item \texttt{exit()}, for detecting spoiler and target instances and for
        checking contract violations;
    \item \texttt{create()}, for creating shadow data for a new thread;
    \item \texttt{acquire()} and \texttt{release()}, for detecting
        synchronization between threads caused by a synchronized method or a
        synchronized statement;
    \item \texttt{preStart()} and \texttt{postJoin()}, for detecting thread
        synchronization caused by the fork or join operation.
\end{itemize}
...
Methods \texttt{exit()} will track target and spoiler instances in a thread, and
...
In order to use immutable objects effectively, the Vavr collection library was
