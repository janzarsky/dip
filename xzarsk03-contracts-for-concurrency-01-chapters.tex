...
program under analysis. Classes are instrumented when they are loaded.
\todo{summary of the rest of the subsection}

For each field, two new methods are added, one for reading and one for writing
to the field. In these methods, write and read events are generated. In the
rest of the class, all \texttt{getfield} and \texttt{putfield} instructions are
replaced with calls to the corresponding methods. For each field, a new field is
created which holds arbitrary data provided by the tools. Figures
\ref{rrInstBefore} and \ref{rrInstField} shows a simple class before and after
...
In RoadRunner, to obtain information about method invocations, most methods are
instrumented by creating a wrapper around them. The original method is renamed,
but otherwise left intact (the code may however be further instrumented to
obtain other information, such as field accesses). Then a wrapper method with
the same name as the original one is created. The wrapper method generates enter
and exit events. In order to detect abnormal method exits that are caused by an
exception being thrown, the call to the original method is wrapped in a try
block. When an exception is caught, the exit method is generated and the
exception is re-thrown. An example of a method instrumented by RoadRunner is
shown in Figure \ref{rrInstMethod}.
...
\subsection{RoadRunner Configuration options}

\todo{what features can be turned on and off}
{\color{blue}\lipsum[2]}
...
\todo{overlapping instances, entry and exit solved together}
{\color{blue}\lipsum[2]}
...
\todo{figure with extended MethodEvent}
{\color{blue}\lipsum[2]}

\todo{changes to the Tool class - method exit, entry}
