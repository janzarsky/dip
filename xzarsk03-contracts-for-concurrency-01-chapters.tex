...
the contract parameter values match. Then finally, the analyzer checks vector
clocks of each matching instance and decides if the two instance interleave each
other.
...
So the analyzer will marked the two instances as interleaving when there was not
...
immutable. Once created, the internal state of objects do not change. All
operations produce new object instead of modifying the current one. See the next
...
Several parts of the analyzer was designed with the dependency inversion
...
analysis are created by tests which allows for tests that do not rely on the
...
Changes to instrumentation were tested by preparing custom subclass of
...
program was created that calls methods with various number and type of
