...
Every analyser in RoadRunner is based on the \texttt{Tool} class. Figure
...
The contract analyser should be implemented as a subclass of the \texttt{Tool}
...
Methods for detecting memory accesses will not be used by the analyser, and also
...
lock, the thread's vector clock is updated. When a lock is released, it's vector
...
In order to properly test the analyser, it's implementation should not rely on
RoadRunner's internal data structures. It should be possible to feed the
analyser a custom event stream without the need for the RoadRunner framework.
